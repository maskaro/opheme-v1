...
class session {
	...
	function start_session($session_name, $secure, $session_id = null) {
	...
	function open() {
		global $db_sec_sess;
		$mysqli = new mysqli($db_sec_sess['host'], $db_sec_sess['username'], $db_sec_sess['password'], $db_sec_sess['dbname']);
		$this->db = $mysqli;
		return true;
	}
	...
	function is_session($id) {
		$this->open();
		if(!isset($this->check_stmt)) {
			$this->check_stmt = $this->db->prepare("SELECT 1 FROM sessions WHERE id = ? LIMIT 1");
		}
		$this->check_stmt->bind_param('s', $id);
		$this->check_stmt->execute();
		$this->check_stmt->store_result();
		$this->check_stmt->bind_result($data);
		$this->check_stmt->fetch();
		$this->close();
	...
	private function encrypt($data, $key) {
		$salt = 'asdkjhAKSJDH^#%IKJH2347@#&*@^lj[pkasd@#!!7i1y23uhjsf21&#@!TIQWKJHasjd';
