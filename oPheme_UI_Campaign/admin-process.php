<?php
	...
	if (!empty($_POST)) {
		...
		switch ($data['action']) {
			case 'resetCampaignMessages':
				$return = $opheme->system_admin_campaignResetMessages($data['campaign_id']);
				break;
			case 'suspendCampaign':
				$return = $opheme->system_admin_campaignSuspend($data['campaign_id']);
				break;
			case 'resumeCampaign':
				$return = $opheme->system_admin_campaignResume($data['campaign_id']);
				break;
			case 'deleteCampaign':
				$return = $opheme->system_admin_campaignDelete($data['campaign_id']);
				break;
			case 'createToken':
